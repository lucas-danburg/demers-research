...
while nn<nmax
    ...
    if ans<10^-8
        ...
        if t0==t1
            ...
            y2=y2+t0*v2*sin(a2);
            % Wrap ball 1
            if x1 > w/2, x1 = x1 - w; elseif x1 < -w/2, x1 = x1 + w; end %Torus addition
            if y1 > l/2, y1 = y1 - l; elseif y1 < -l/2, y1 = y1 + l; end %Torus addition
            % Wrap ball 2
            if x2 > w/2, x2 = x2 - w; elseif x2 < -w/2, x2 = x2 + w; end %Torus addition
            if y2 > l/2, y2 = y2 - l; elseif y2 < -l/2, y2 = y2 + l; end %Torus addition
        else
            flag=2;
            t1=t1-t0;
