%
%lemonlyap computes the Lyapunov Exponent for Lemon
%billiard tables
%

function limArr=lemonlyap(delta,derivComp,n,raw)
...
%function for the lemon at xo and x1
derivfxo=0.5*(2*delta*xo/(sqrt(1+delta*(delta-2)*(1-x^2))));
derivfx1=0.5*(2*delta*x1/(sqrt(1+delta*(delta-2)*(1-x^2))));

%curvature of the lemon at xo and x1
curvatureo=abs(derivfxo)/(sqrt(1+derivfxo^2)^3);
curvature1=abs(derivfx1)/(sqrt(1+derivfx1^2)^3);

%computes the derivative matrix
if isnan(ao) || isnan(a1) && x~=0 %in case the angle is passed as a NaN
