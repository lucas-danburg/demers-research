%
%mushroomlyap computes the Lyapunov Exponent for Mushroom
%billiard tables
%

function limArr=mushroomlyap(rx,ry,derivComp,n,circ,cx,cy,raw)
...
if isnan(ao) || isnan(a1) %in case the angle is passed as a NaN
    ...
    if piece0==6 %if r0 is on the circle/ellipse
        ...
        if piece1==6  %if r1 is on the circle/ellipse
        ...
        if piece1==6  %r1 is on circle/ellipse
            if circ==1 %if circular mushroom
               curvature1=-1/rx;
            else    %if elliptical mushroom
               if x1-cx==0
