...
if abs(mod(ao,pi)-pi/2)<10^-2  %deals specially with vertical trajectories
    %disp('vertical trajectory (10^-2 precision)')
    for k=1:size(z,2)
        if (table{piece(z(k), handles.table),2}(z(k))-yo)*sin(ao)<0    %if root corresponds to opposite direction
    ...
    for k=1:size(z,2)   %deals with all non-vertical trajectories
        if (table{piece(z(k), handles.table),1}(z(k))-xo)*cos(ao)<-5*10^-6    %if root corresponds to opposite direction
            %disp('opposite direction for root:')
            %disp(z(k))
            remove=[remove,k]; 
        end
...
data(n,4)=piece(told, handles.table); %which piecewise function of the table t is located in
