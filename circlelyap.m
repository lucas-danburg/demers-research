%
%circlelyap computes the Lyapunov Exponent for Circle
%billiard tables
%

function limArr=circlelyap(r,derivComp,n,raw)
...
if isnan(ao) || isnan(a1) %in case the angle is passed as a NaN
    ...
    derivTable(x,1)=(-1/cos(a1))*(tau*curvature+cos(ao));
    derivTable(x,2)=-(tau)/cos(a1);
    derivTable(x,3)=(-1/cos(a1))*(tau*curvature*curvature...
        +curvature*cos(a1)+curvature*cos(ao));
    derivTable(x,4)=(-1/cos(a1))*(tau*curvature+cos(a1));
end
