%
%roundedmushroomlyap computes the Lyapunov Exponent for Rounded Mushroom
%billiard tables
%

function limArr=roundedmushroomlyap(rx,ry,derivComp,n,circ,cx,cy,raw)
...
if isnan(ao) || isnan(a1) %in case the angle is passed as a NaN
    ...
    if piece0==6 || piece0==2 %if r0 is on the circle/ellipse
        
        %curvature at point r0
        if circ==1
            curvatureo=-1/rx;
        else
            if xo-cx==0 
        ...
        %the curvature at point r1
        if piece1==6 || piece1==2 %if r1 is on the circle/ellipse
        ...
        else    %r1 is on flat surface
            curvature1=0;
        end
    else    %if r0 is on flat surface
        curvatureo=0;

        %curvature at point r1
        if piece1==6 || piece1==2 %r1 is on circle/ellipse
            if circ==1 %if circular mushroom
               curvature1=-1/rx;
            else    %if elliptical mushroom
               if x1-cx==0
        ...
        else    %if r0 and r1 are on flat surface
            curvature1=0;
        end
    end
