%
%nonconcirclyap computes the Lyapunov Exponent for Non Concentric Circle
%billiard tables
%

function limArr=nonconcirclyap(rx1,rx2,derivComp,n,raw)
...
if isnan(ao) || isnan(a1) %in case the angle is passed as a NaN
    ...
    if piece0==1 %if r0 is on the outer circle
        curvatureo=-1/rx1;
        if piece1==1
            curvature1=-1/rx1;
        else
    ...
    else %if r0 is on the inner circle
        curvatureo=1/rx2;
        if piece1==1
            curvature1=-1/rx1;
        else
